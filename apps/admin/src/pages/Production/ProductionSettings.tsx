...
const ProductionSettings = () => {
  ...
  useEffect(() => {
    ...
    // Fetch production settings and overview from the server
    const fetchSettingsAndOverview = async () => {
      try {
        const response = await AxiosInstance.get(`/production/${userId}`);
  ...
  const handleUpdateSettings = async (e: React.FormEvent) => {
    ...
    try {
      // Send the settings object in the correct format
      await AxiosInstance.patch(`/production/${userId}/password`, { password: settings.password });
    ...
    <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold text-black dark:text-white">
          {t('Production Settings')}
        </h2>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {t('Manage production preferences and configurations')}
        </p>
      </div>
