...
const AddDailyModal: React.FC<AddDailyModalProps> = ({ onClose, onAdd, initialTransportId, deliveryId }) => {
  ...
  useEffect(() => {
    ...
    const fetchTransports = async () => {
      try {
        const response = await axiosInstance.get(`/delivery/${deliveryId}`);
        console.log('API Response:', response.data);
    
        if (!response.data || !response.data.transport) {
          toast.error('Transport data is not available');
          return;
        }
        ...
        // Ensure derivedResponse.data is an array before using reduce
        const derivedData = Array.isArray(response.data) ? response.data : [];
        const derivedTotal = derivedData.reduce((sum: number, item: any) => sum + (item.amount || 0), 0);
