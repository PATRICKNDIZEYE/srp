...
const AssignedDeliveries = () => {
  ...
  useEffect(() => {
    const fetchDeliveries = async () => {
      if (!transportId) {
        console.log('No transportId found');
        return;
      }
      try {
        console.log('Fetching deliveries for transportId:', transportId);
        const response = await axiosInstance.get(`/transportations/transport/${transportId}`);
  ...
  const handleViewDaily = (deliveryId: string) => {
    navigate(`/management/daily-management/${deliveryId}`);
  };
