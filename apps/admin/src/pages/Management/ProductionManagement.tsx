...
const ProductionManagement = () => {
  ...
  const userId = transportId ? parseInt(transportId, 10) : null;
  const [transporterName, setTransporterName] = useState('Transportation Management');
  ...
  useEffect(() => {
    const fetchTransporterName = async () => {
      if (!userId) return;
      try {
        const response = await axiosInstance.get(`/transportations/${userId}`);
        setTransporterName(response.data.name || 'Transportation Management');
      } catch (error) {
        console.error('Error fetching transporter name:', error);
        toast.error('Failed to fetch transporter name');
      }
    };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      <Breadcrumb pageName={transporterName} />
